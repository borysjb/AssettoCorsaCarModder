...
std::map <std::string,std::string> engineExplanations {
    ...
    {"Redline RPM","Max RPM the engine will rev to by itself (Limiter will kick in) [Integer value]"},
    {"Limiter HZ","How often the limiter stops the engine, in Hertz (amount of times per second). Less equals more \"choppy\" limiter bang [Integer value]"},
    {"Coast Reference RPM","The RPM at which coast torque is the given amount [Integer value]"},
    {"Coast Torque","The torque the engine produces while coasting, at the Reference RPM. More = harsher deceleration on lift-off [Integer value]"},
    {"Coast Nonlinearity","How linear the coast curve is. 0 = A line from 0 torque at 0 RPM to Coast Torque at Coast Reference RPM, >0 is some exponential function (value 0 recommended) [Value betheen 0 and 1]"},
    ...
    {"Max Boost","Maximum theoretical boost pressure, in bar"},
    {"Wastegate Pressure","controls the maximum allowed boost pressure, in bar. It is not the real-world wastegate spring pressure or anything similar, but a practical max pressure. Refer to “car physics” app to troubleshoot in-game boost pressure"},
    {"Max Display Boost (for use in game UI)","the boost pressure considered to be maximum by display apps in-game."},
    {"RPM at which max boost is reached","controls the theoretical earliest RPM where MAX_BOOST can be attained, at full throttle with no lag. [Integer value]"},
    ...
    {"Boost adjustable from cockpit?", "Controls if turbos are adjustable while driving [Value 0 or 1]"},
    ...
    {"Engine damage threshold RPM","controls RPM before engine takes damage. Example: 9000, [Integer value]"},
